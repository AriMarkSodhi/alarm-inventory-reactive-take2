...
public class AlarmPopulationApp implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        alarmDefinitionDAL.findAll().parallel(2).runOn(Schedulers.parallel()).subscribe(value -> LOG.info(value.getName()));
        alarmDefinitionDAL.findAll(PageRequest.of(0, 2)).subscribe(value -> LOG.info(value.getName()));
        ...
        alarmDAL.findAll().parallel(2).runOn(Schedulers.parallel()).subscribe(value -> LOG.info(value.getName()));
        alarmDAL.findAll(PageRequest.of(0, 2)).subscribe(value -> LOG.info(value.getName()));
    }
}
